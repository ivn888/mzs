...
def formOpen(dialog,layerid,featureid):
    ...
    global LedSpessore, LedTop, LedBottom, LedQtop, LedQbottom, BtnFileTabella
    global LedTabCurve
    ...
    ComboBoxTipoPar = myDialog.findChild(QComboBox,"tipo_parlncmb")
    ...
    LedTop.cursorPositionChanged.connect(updSpessore)
    LedBottom.cursorPositionChanged.connect(updSpessore)
    LedQtop.cursorPositionChanged.connect(ctrlquota)
    LedQbottom.cursorPositionChanged.connect(ctrlquota)
...
# legge i dati dalla tabella di decodifica 'tab_parametri_pun' in rsTabParametri
    ...
    aLayer = QgsMapLayerRegistry.instance().mapLayersByName('tab_parametri_lin')[0]
    
    # recupero gli attributi delle features
    lista=[]
    rsTabParametri=[] # vettore contenente la tabella 'tab_parametri_lin'
